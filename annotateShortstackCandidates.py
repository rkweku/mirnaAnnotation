...
resultsFilename = "/home/rkweku/miRador_Testing/ShortStack/AT_pub2_sRNA_results/Results.txt"
countsFilename = "/home/rkweku/miRador_Testing/ShortStack/AT_pub2_sRNA_results/Counts.txt"
mergedFilename = "/home/rkweku/miRador_Testing/ShortStack/AT_pub2_sRNA_results/ResultsCountsMerged.csv"
summaryFilename = "/home/rkweku/miRador_Testing/ShortStack/AT_pub2_sRNA_results/summary.txt"
fastaFilename = "%s.fa" % os.path.splitext(mergedFilename)[0]
annotatedFilename = "/home/rkweku/miRador_Testing/ShortStack/AT_pub2_sRNA_results/ResultsCountsMerged-annotated.csv"
preAnnotationFilename = mergedFilename

def mergeShortstackResultsAndCounts(resultsFilename, countsFilename,
        outputFilename, fastaFilename):
    """Filter only confirmed miRNAs predicted by Shortstack and merge
    the library abundances for each file

    Args:
        resultsFilename: The results output file from shortstack
        countsFilename: The counts output file from shortstack
        outputFilename: The name of the combined files
        fastaFilename: The name of the fasta file

    """

    # Create empty array to hold full output file
    wholeFile = []
    lineCounter = 0

    # Read the full counts file
    with open(countsFilename, "r") as f:
        countsFile = f.readlines()

    # loop through file using csv.reader and store in wholeFile
    with open(resultsFilename, "r") as f, open(mergedFilename, "w") as g,\
            open(fastaFilename, "w") as h:
        reader = csv.reader(f, delimiter = "\t")
        # Skip the header
        next(reader, None)
        g.write("miR Name,Chr,Strand,miR Start, miR End,miR Sequence,")

        libList = countsFile[0].rstrip().split("\t")[3:]
        for lib in libList:
            if(lib == libList[-1]):
                g.write("%s\n" % lib)
            else:
        ...
        for row in reader:
            ...
            if(mirnaFlag == "Y"):
                ...
                g.write("%s,%s,%s,%s,%s,%s," % (name, chr, strand, start,
                    end, mirSeq))
                h.write(">%s\n%s\n" % (name, mirSeq))

                abundanceList = countsFile[lineCounter].rstrip().split("\t")[3:]
                for i in range(len(abundanceList)):
                    if(i == len(abundanceList) - 1):
                        g.write("%s\n" % abundanceList[i])
                    else:
                        g.write("%s," % abundanceList[i])
        ...
        for identicalMirna in identicalList:
            ...
            for coordinates in mirBaseDict[identicalMirna]:
                ...
                # the issue to the user and continue to the next tag
                    ...
                    print("Unrecognized strand of miRBase entry. We "\
                        "will skip this entry, but please check with the "\
                        "miRBase file %s.gff3 and miRNA name %s" % (organism,
                        identicalMirna))
                    continue

                # Shortstack does not provide the candidate miRNA's position
                # but rather the entire precursor range, so if the identical
                # miRBase falls within the Shortstack predicted precursor,
                # then we will call this a known miRNA
                if(chrName == mirBaseChr and strand == mirBaseStrand and
                        start <= mirBasePosition and end >= mirBasePosition):
...
def annotateShortstackCandidates(similarityDict, organism, mirBaseDict,
        numLibs):
    ...
    for line in preAnnotatedFile[1:]:
        mirName = line[header.index("miR Name")]
        libCount = 0
        identicalFlag = False
        similarFlag = False
        ...
        if(mirName in similarityDict):
            ...
            if(isinstance(similarityDict[mirName], list)):
                ...
                line = annotateIdenticalCandidates(
...
def createSummary(classificationCountsList):
...
def main():
    ...
    mergeShortstackResultsAndCounts(resultsFilename, countsFilename,
        mergedFilename, fastaFilename)
    ...
    addSequencesToOutput(fastaFilename,
    ...
    classificationCountsList = annotateShortstackCandidates(similarityDict,
        organism, mirBaseDict, numLibs)

    createSummary(classificationCountsList)
